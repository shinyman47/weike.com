...
if($_SESSION['username']==''){
    ...
    $sql="select * from mark WHERE username=$_SESSION[username] AND lesson_id=$_SESSION[ID]";
    if($result=mysqli_query($conn, $sql)){
        echo "<script type='text/javascript'>alert('您已经发表过评论');location='javascript:history.back()';</script>";
    }
    else{
        $sql2 = "insert into mark(username,lesson_id,mark,ys) values('$_SESSION[username]',$_SESSION[ID],'$text',0) ";
